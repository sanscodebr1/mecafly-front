...
// Componente para mostrar o motivo da rejeição
  ...
  return (
    <View style={styles.reasonContainer}>
      ...
      <Text style={styles.reasonText}>
        {showFullReason ? reason : shortReason}
      </Text>
      {reason.length > 80 && (
        <TouchableOpacity 
...
export function DocumentsScreen() {
  ...
  const handleDocumentUpload = async (documentType: DocumentType) => {
    try {
      ...
      // Upload usando o serviço corrigido
  ...
  const handleDocumentPreview = async (document: UserDocument) => {
    try {
      ...
      // Usar o método getPublicUrl do serviço
      const publicUrl = documentsService.getPublicUrl(document.url);
      
      console.log('URL pública do documento:', publicUrl);
      console.log('Path do documento:', document.url);
  ...
  const handleResubmitDocument = async (document: UserDocument) => {
    try {
      ...
      // Verificar arquivo antes do reenvio
      const fileInfo = await FileSystem.getInfoAsync(asset.uri);
      if (!fileInfo.exists) {
        throw new Error('Arquivo não existe');
      }

      // Reenviar usando o serviço corrigido
                ...
                {previewUrl.toLowerCase().includes('.pdf') ? (
                  <View style={styles.pdfContainer}>
                    ...
                    <TouchableOpacity 
                      ...
                      onPress={() => {
                        // Aqui você pode usar Linking.openURL(previewUrl) se necessário
...
const styles = StyleSheet.create({
  ...
  title: {
    ...
    color: '#000000',
    marginBottom: hp('2.5%'),
    ...(isWeb && { fontSize: wp('3.5%'), marginBottom: hp('2%') }),
  },
  documentCard: {
